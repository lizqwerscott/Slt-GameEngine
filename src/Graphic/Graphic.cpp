...
{
	this->m_window = std::shared_ptr<sf::RenderWindow>(new sf::RenderWindow(sf::VideoMode(width, height), tile.c_str()));
...
sf::Color 
Graphic::changeColor(b2Color color) {
	return sf::Color((int)(color.r * 255), (int)color.g * 255, (int)color.b * 255, (int)color.a * 255);
}
...
void Graphic::insertKeyCallBack(sf::Keyboard::Key key, std::function<void(void)> keyCallBack) {
...
void Graphic::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
{
	...
	{
		sf::Vertex point(Math::PhysicalCoordSToDrawCoordS(center) + sf::Vector2f(cos(unitR * i) * drawRadius, sin(unitR * i) * drawRadius));
		point.color = Graphic::changeColor(color);
		v.push_back(point);
	}
	sf::Vertex point(Math::PhysicalCoordSToDrawCoordS(center) + sf::Vector2f(cos(0) * drawRadius, sin(0) * drawRadius));
	point.color = Graphic::changeColor(color);
	v.push_back(point);
	graphic->m_polygons.push_back(v);
}
...
void Graphic::DrawSolidCircle(const b2Vec2& center, float32 radius, const b2Vec2& axis, const b2Color& color)
{
	...
	circle->setPosition(pos - sf::Vector2f(Math::MeterToPixel(radius), Math::MeterToPixel(radius)));
...
sf::ConvexShape *
Graphic::GetShape(b2PolygonShape * polygonShape, b2Vec2 pos, sf::Color color)
{
	sf::ConvexShape * convex = new sf::ConvexShape();
	convex->setPointCount(polygonShape->GetVertexCount());
	for (int i=0; i < polygonShape->GetVertexCount(); i++)
	{
		//printf("PolygonShape:Point:%d:%f, %f\n", i, polygonShape->GetVertex(i).x, polygonShape->GetVertex(i).y);
		convex->setPoint(i, Math::PhysicalCoordSToDrawCoordS(polygonShape->GetVertex(i) + pos));
		//printf("Convex:Point:%d:%f, %f\n", i, convex->getPoint(i).x, convex->getPoint(i).y);
	}
	convex->setFillColor(color);
	//printf("Convex:Pos:%f, %f\n", convex->getPosition().x, convex->getPosition().y);
	return convex;
} 

sf::CircleShape *
Graphic::GetShape(b2CircleShape * circleShape, sf::Color color)
{
	sf::CircleShape * circle = new sf::CircleShape(Math::MeterToPixel(circleShape->m_radius));
	circle->setFillColor(color);
	sf::Vector2f pos = Math::PhysicalCoordSToDrawCoordS(circleShape->m_p);
	circle->setPosition(pos - sf::Vector2f(Math::MeterToPixel(circleShape->m_radius), Math::MeterToPixel(circleShape->m_radius)));
	return circle;
}

sf::Shape *
Graphic::GetShape(b2Fixture * fixture, sf::Color color)
{
	...
	case b2Shape::e_circle:
	{
		...
		sf::CircleShape * circleS = new sf::CircleShape(Math::MeterToPixel(circleP->m_radius));
		circleS->setFillColor(color);
		circleS->setOrigin(Math::MeterToPixel(circleP->m_radius), Math::MeterToPixel(circleP->m_radius));
	...
	case b2Shape::e_polygon:
	{
		...
		{
			//printf("PolygonShape:Point:%d:%f, %f\n", i, polygonP->GetVertex(i).x, polygonP->GetVertex(i).y);
			convexS->setPoint(i, sf::Vector2f(Math::MeterToPixel(body->GetWorldPoint(polygonP->GetVertex(i)).x), Math::MeterToPixel(body->GetWorldPoint(polygonP->GetVertex(i)).y)));
			//printf("Convex:Point:%d:%f, %f\n", i, convexS->getPoint(i).x, convexS->getPoint(i).y);
		}
