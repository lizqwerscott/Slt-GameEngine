...
SNode::SNode(std::string name, SNode * parent) 
	: Node<SNode>(parent), Object(name), m_physicalBody(nullptr), m_posation(b2Vec2(0.0f, 0.0f))
{
}
...
void SNode::Draw()
{
	...
	if (this->m_isDraw)
	{
		...
		for (auto sprite : this->m_sprites)
		{
			...
			sprite.second->setPosition(Math::WorldCoordSToDrawCoordS(this->m_posation));
		...
		for (auto shape : this->m_shapes)
		{
			...
			shape.second->setPosition(Math::WorldCoordSToDrawCoordS(this->m_posation));
...
b2Vec2 
SNode::GetPosation() {
	return this->m_posation;
}

void SNode::SetPosation(b2Vec2 pos) {
	this->m_posation = pos;
	if (this->m_physicalBody != nullptr)
...
void SNode::move(b2Vec2 posOffset) {
	this->SetPosation(this->m_posation + posOffset);
}
...
std::shared_ptr<PhysicalBody>
SNode::CreatePhysicalBody(std::string name, b2Vec2 localWorldPos, b2BodyDef bodyDef, b2World * world)
{	
	this->m_physicalBody = std::shared_ptr<PhysicalBody>(new PhysicalBody(name, localWorldPos, bodyDef, world));
...
SNode::GetPhysicalBody() {
	if (this->m_physicalBody == nullptr)
	{
		printf("SNode::GetPhysicalBody()->[Warring]:The PhyicalBody is nullptr\n");
		return nullptr;
	}
	else
	{
