...
void MyBreakoutScene::init()
{
    ...
    this->GetRootNode()->CreateChild(std::string("Wall"), [windowSize, physicalWorld](SNode * node)->void {
        ...
        bodyDef.angle = 0;
        auto physicalBody = node->CreatePhysicalBody(std::string("WallBody"), b2Vec2(0, 0), bodyDef, physicalWorld.get());
        //Fixture
        ...
        b2Vec2 upLocalWorldPos(Math::DrawCoordSToPhysicalCoords(sf::Vector2f(windowSize.x / 2, 0)));
        b2PolygonShape upPolygonShape;
        upPolygonShape.SetAsBox(Math::PixelToMeter(windowSize.x / 2), 1, Math::WorldCoordSToPhysicalCoordS(upLocalWorldPos), 0);
        ...
        b2Vec2 downLocalWorldPos(Math::DrawCoordSToPhysicalCoords(sf::Vector2f(windowSize.x / 2, windowSize.y)));
        b2PolygonShape downPolygonShape;
        downPolygonShape.SetAsBox(Math::PixelToMeter(windowSize.x / 2), 1, Math::WorldCoordSToPhysicalCoordS(downLocalWorldPos), 0);
        ...
        b2Vec2 leftLocalWorldPos(Math::DrawCoordSToPhysicalCoords(sf::Vector2f(0, windowSize.y / 2)));
        b2PolygonShape leftPolygonShape;
        leftPolygonShape.SetAsBox(1, Math::PixelToMeter(windowSize.y / 2), Math::WorldCoordSToPhysicalCoordS(leftLocalWorldPos), 0);
        ...
        b2Vec2 rightLocalWorldPos(Math::DrawCoordSToPhysicalCoords(sf::Vector2f(windowSize.x, windowSize.y / 2)));
        b2PolygonShape rightPolygonShape;
        rightPolygonShape.SetAsBox(1, Math::PixelToMeter(windowSize.y / 2), Math::WorldCoordSToPhysicalCoordS(rightLocalWorldPos), 0);
    ...
    */
    ...
    /*
    EventHandlerIml * eventHandler = new EventHandlerIml(SNode::EventSNodeType::ES_UpdateBegin, [](Event * evt, EventSender * sender)->void {
        //printf("SNode::EventSNodeType::ES_UpdateBegin:Type:%d", ((EventSNodeUpdateBegin *)evt)->m_type);
    }, ballNode);
    EventManager::Instance()->AddEventHandler(SNode::EventSNodeType::ES_UpdateBegin, eventHandler);
    */
    ...
    ballNode->pushUpdateCallBack([this, baffleNode, windowSize](SNode * node)->void {
    	b2Vec2 ballPos = node->GetPosation();
    	sf::CircleShape * ballShape = dynamic_cast<sf::CircleShape *>(node->GetShape("MainCircle").get());
    	//Collision in wall
    	...
    	{
    		//sf::RectangleShape * rectangleShape = dynamic_cast<sf::RectangleShape *>(baffleNode->GetShape("MainRectangle").get());
    	...
    	//printf("Speed:%f, %f\n", speed.x, speed.y);
    	b2Vec2 movePos(speed.x + node->GetPosation().x, abs(speed.y - node->GetPosation().y));
    	//printf("MovePos:%f, %f\n", movePos.x, movePos.y);
